...
export const ProductInfo = ({
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<Star key={i} className={styles.star_full} />);
    }
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<Star key={`empty-${i}`} className={styles.star_empty} />);
    }
  ...
  return (
    <div className={styles.product_info}>
      ...
      <div className={styles.rating_section}>
        <div className={styles.stars}>{renderStars(rating || 0)}</div>
      ...
      <div className={styles.price_section}>
        <div className={styles.current_price}>{formatPrice(price)}</div>
      ...
      <div className={styles.stock_section}>
        <span
          className={`${styles.stock_status} ${inStock ? styles.in_stock : styles.out_stock}`}
        >
